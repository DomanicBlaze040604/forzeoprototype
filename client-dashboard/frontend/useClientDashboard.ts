 ...
 * 
 * This hook manages all state and logic for the AI Visibility Dashboard.
 * 
 * Features:
 * - Multi-tenant client management (add/edit/delete)
 * - Unlimited prompts with multi-format import
 * - Real-time audit execution across multiple AI models
 * - Cost tracking per model and prompt
 * - LocalStorage persistence per client
 * - Export reports in multiple formats
 * - AI-powered prompt and content generation
 * 
 * @example
 * const { clients, selectedClient, runAudit, ... } = useClientDashboard();
 */

import { useState, useCallback, useEffect } from "react";
import { supabase } from "@/integrations/supabase/client";
import {
  AI_MODELS, INDUSTRY_PRESETS, LOCATION_CODES,
  type Client, type Prompt, type AuditResult, type ModelResult,
  type DashboardSummary, type SourceSummary, type CostBreakdown
} from "./types";

// Re-export for convenience
export { AI_MODELS, INDUSTRY_PRESETS, LOCATION_CODES };
export type { Client, Prompt, AuditResult, ModelResult, DashboardSummary, SourceSummary, CostBreakdown };
...
export function useClientDashboard() {
  ...
  /**
   * Update selected models and persist to storage
   */
  ...
  /**
   * Initialize clients from storage or create defaults
   */
  const fetchClients = useCallback(async () => {
    const storedClients = loadFromStorage<Client[]>(STORAGE_KEYS.CLIENTS, []);
    
    if (storedClients.length === 0) {
      // Create default pilot clients
      ...
      // Select last used or first client
  ...
  /**
   * Delete a client (must keep at least one)
   */
  const deleteClient = useCallback((clientId: string) => {
    ...
    // Clean up prompts and results for deleted client
    const storedPrompts = loadFromStorage<{ [key: string]: Prompt[] }>(STORAGE_KEYS.PROMPTS, {});
  ...
  /**
   * Clear all prompts for current client
   */
  ...
  /**
   * Load audit results for a client
   */
  ...
  /**
   * Save audit results for a client
   */
  ...
  /**
   * Clear all results for current client
   */
  ...
  /**
   * Update brand tags for current client
   */
  ...
  /**
   * Update competitors for current client
   */
  ...
  const runAudit = useCallback(async (promptText: string, promptId?: string): Promise<AuditResult | null> => {
    ...
    try {
      // Call the geo-audit Edge Function
      ...
      // Update state (replace existing result for same prompt)
  ...
  /**
   * Run audit for all prompts that don't have results
   */
  ...
  /**
   * Re-run audit for a specific prompt (replaces existing result)
   */
  ...
  /**
   * Update cost breakdown from results
   */
  ...
  /**
   * Update dashboard summary from results
   */
  const updateSummary = useCallback((results: AuditResult[]) => {
    ...
    for (const result of results) {
      ...
      for (const mr of result.model_results) {
        ...
        for (const c of mr.citations) {
          if (!sourceMap.has(c.domain)) {
            sourceMap.set(c.domain, { count: 0, urls: new Set(), prompts: new Set() });
          }
          const entry = sourceMap.get(c.domain)!;
          entry.count++;
          entry.urls.add(c.url);
          entry.prompts.add(result.prompt_text);
        }

        // Aggregate competitor data
        for (const comp of mr.competitors_found) {
          if (!competitorMap.has(comp.name)) {
            competitorMap.set(comp.name, { mentions: 0, ranks: [] });
          }
    ...
    const overallSov = totalChecks > 0 ? Math.round((totalVisible / totalChecks) * 100) : 0;
    ...
    setSummary({
      ...
      top_sources: Array.from(sourceMap.entries())
        .sort((a, b) => b[1].count - a[1].count)
        .slice(0, 10)
        .map(([domain, data]) => ({ domain, count: data.count })),
      top_competitors: Array.from(competitorMap.entries())
    ...
    // Set source summary with full URLs
    setSourceSummary(
      Array.from(sourceMap.entries())
        .sort((a, b) => b[1].count - a[1].count)
        .map(([domain, data]) => ({
          domain,
          full_urls: Array.from(data.urls),
  ...
  const exportToCSV = useCallback(() => {
    ...
    downloadFile(
      `${selectedClient.slug}-audit-${new Date().toISOString().split("T")[0]}.csv`,
  ...
  const exportPrompts = useCallback(() => {
    ...
    downloadFile(
      `${selectedClient.slug}-prompts-${new Date().toISOString().split("T")[0]}.json`,
  ...
  const exportFullReport = useCallback(() => {
    ...
    allCitations.forEach((citation, idx) => {
      lines.push(`${idx + 1}. ${citation.title || citation.domain}`);
    ...
    downloadFile(
      `${selectedClient.slug}-visibility-report-${new Date().toISOString().split("T")[0]}.txt`,
      lines.join("\n"),
      "text/plain"
    );
  }, [selectedClient, auditResults, summary, sourceSummary]);
    ...
    try {
      ...
      if (data.prompts && Array.isArray(data.prompts)) {
        const texts = data.prompts
          .map((p: any) => typeof p === "string" ? p : p.text || p.prompt_text)
          .filter(Boolean);
  ...
  /**
   * Import prompts from plain text (one per line)
   */
...
Format in clean Markdown.`,
  ...
  /**
   * Get all citations across all results (aggregated)
   */
